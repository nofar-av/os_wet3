...
void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg,  Stats* thread_stats) 
{
   char buf[MAXLINE], body[MAXBUF];
   ...
   sprintf(buf, "Content-Length: %lu\r\n", strlen(body));
   sprintf(buf, "%sStat-Req-Arrival:: %lu.%06lu\r\n", buf, thread_stats->arrival_time.tv_sec, thread_stats->arrival_time.tv_usec);
   sprintf(buf, "%sStat-Req-Dispatch:: %lu.%06lu\r\n", buf, thread_stats->wait_time.tv_sec, thread_stats->wait_time.tv_usec);
...
void requestServeDynamic(int fd, char *filename, char *cgiargs, Stats* thread_stats)
{
...
void requestServeStatic(int fd, char *filename, int filesize, Stats* thread_stats) 
{
   int srcfd;
...
void requestHandle(int fd, Stats* thread_stats)
{
   ...
   if (stat(filename, &sbuf) < 0) {
      requestError(fd, filename, "404", "Not found", "OS-HW3 Server could not find this file", thread_stats);
      return;
   }
